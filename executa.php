...
#$solucoes ['Go-SergioCorreia_V2'] 	= ['exec'=>'./Go-SergioCorreia_V2','language'=>'Go','creator'=>'Sérgio Correia','source'=>'https://github.com/OsProgramadores/op-desafios/blob/master/desafio-05/qrwteyrutiyoup/go'];
...
#$solucoes ['Php-Bcampos_V2']		= ['exec'=>'/usr/bin/php Php-Bcampos_V2.php','language'=>'Php','creator'=>'Bernardino Campos','source'=>'https://github.com/BernardinoCampos/Desafio5/blob/master/Desafio5.php'];
...
foreach ($tamanhos as $tam=>$v) {
	...
	for($aa=0;$aa<$numExecucoes;$aa++)
		foreach ($solucoes as $key => $vector) {
			...
			$cmd = "/usr/bin/cgexec -g memory:OsProgramadores {$vector['exec']} ./HD/{$arquivo} | /usr/bin/sort | sed '/^\$/d' | /usr/bin/md5sum";
			$fd = popen($cmd, 'r');
